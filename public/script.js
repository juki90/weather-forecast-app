...
function setWeatherData(data) {
  ...
  var inElement = document.querySelector(".result-today"),
    ...
    location = elementCreation(
      "today-location",
      `Location: <b class="b">${data.name}, ${data.sys.country}</b>`
    ),
    description = elementCreation(
      "today-description",
      `Description: <b class="b">${data.weather[0].main} (${data.weather[0].description})</b>`
    ),
    temperature = elementCreation(
      "today-temperature",
      `Temperature: <b class="b">${data.main.temp} ${tempDegree}</b>`
    ),
    humidity = elementCreation(
      "today-humidity",
      `Humidity: <b class="b">${data.main.humidity}%</b>`
    ),
    wind_speed = elementCreation(
      "today-wind_speed",
      `Wind speed: <b class="b">${data.wind.speed} ${speed}</b>`
    ),
    pressure = elementCreation(
      "today-pressure",
      `Air pressure: <b class="b">${data.main.pressure} hPa</b>`
    ),
    arr = [location, description, temperature, humidity, wind_speed, pressure];
    ...
    svg
      ...
      .attr("transform", function(d, i) {
        if (i !== 0) {
          ...
          switch (domain) {
            case "temperature":
              return `translate(${(i * width - 90) / data.length}, ${height -
                170.5 -
                tempValue * 1.2})`;
            case "humidity":
              return `translate(${(i * width - 140) / data.length}, ${height -
                20 -
                d.value * 3.2})`;
            case "airpressure":
              return `translate(${(i * width - 140) / data.length}, ${1315 -
                d.value * 1.2})`;
            case "windspeed":
              return `translate(${(i * width - 140) / data.length}, ${height -
                10 -
...
function setError() {
  let errorElement = document.createElement("p"),
    weatherContainer = document.querySelector(".result-today"),
    forecastContainer = document.querySelector(".result-forecast"),
    infoContainer = document.querySelector(".search-info");

  errorElement.classList.add("error-text");
  errorElement.innerText = "Error loading data, try again in few seconds";
